...
- The `onSetup` apiService function would not run when defining custom group toolbar button. #TINY-9496
...
- Fixed the customeditor component not able to get data from the dialog apiService. #TINY-3866
...
- Added new apiService endpoints to the wordcount plugin and improved character count logic. #TINY-3578
...
- Fixed a bug with the annotations apiService where annotating collapsed selections caused problems. #TBS-2449
...
- Fixed a bug where the Inlite Theme didn't use the Table Plugin apiService to insert correct tables.
...
- Added a new apiService for providing meta data for plugins. It will show up in the help dialog if it's provided.
...
- Fixed bug where the I18n apiService was not supporting arrays as the translation replacement values.
...
- Fixed bug where various apiService calls would be throwing exceptions if they where invoked on a removed editor instance.
...
- Added apiService to the list plugin for setting custom classes/attributes on lists.
...
- Added new sidebar apiService that allows you to add custom sidebar panels and buttons to toggle these.
...
- Fixed various apiService documentation issues and typos.
